<?php
	...
	function show_latest_public_photo(){
		...
		if($stmt->fetch()){
			...
			$photo_html = '<img src="show_public_photo.php?photo=' .$id_from_db .'" alt="'; //.$GLOBALS["photo_upload_normal_dir"] .$filename_from_db .'" alt="';
			if(empty($alttext_from_db)){
				$photo_html .= "Üles laetud foto";
			} else {
			...
			$photo_html = "<p>Kahjuks ei ole ühtegi avalikku fotot üles laetud</p>";
		}
	...
	function read_public_photo_thumbs($privacy, $page, $limit){
		...
		while($stmt->fetch()){
			//<div>
			//<img src="kataloog/fail" alt="text" class="thumbs" data-id="x" data-fn(sen filename) ="failinimi.jpg">
			//<img src="show_public
			//</div>
	...
	function read_own_photo_thumbs($page, $limit){
		...
		while($stmt->fetch()){
			//<div>
			//<a ref="edit_photo.php?photo=x">
			//<a href="edit_gallery_photo.php?photo=x">
			//</a>
			//<img src="kataloog/fail" alt="text">
			//</a>
			//...
			//</div>
			$photo_html .= '<div class="gallerythumb">' ."\n";
